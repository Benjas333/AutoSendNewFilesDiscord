...
def globNewFiles(directory: str, extension: str | list[str], recursive: bool = False):
        ...
        all_files = list(map(lambda file: str(file), chain.from_iterable((directory_path.glob(match_string) for match_string in match_strings) if not recursive else directory_path.rglob(match_string) for match_string in match_strings)))
